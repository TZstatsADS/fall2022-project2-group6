...
covid <- read_csv("covid_hosp\\COVID-19_Daily_Counts_of_Cases__Hospitalizations__and_Deaths.csv")
head(covid)
...
server <- function(input, output) {
  ...
  output$plot=renderPlot({
    covid_date <- covid %>% mutate(date_of_interest = as.POSIXct(date_of_interest, format="%m/%d/%Y")) %>%
